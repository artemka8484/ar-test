...
def home():
    ...
    if qr_id not in db:
        return redirect("https://t.me/my_magic_ar_bot?start=" + str(qr_id))
...
"
if data.get('vid'):
h = h + "🎥 Видео: " + str(data['vid']) + "


"
if data.get('tst'):
h = h + "🥂 Тосты: " + ", ".join(data['tst']) + "


"
if data.get('gam'):
h = h + "🎮 Игра: " + str(data['gam']) + "
...
if data.startswith("lang_"):
    ...
    bot.edit_message_text("Главное меню:", uid, call.message.message_id, reply_markup=get_hub_menu(uid))
elif data == "hub_reset":
    qr_id = users[uid]['qr']
    users[uid] = {'qr': qr_id, 'lang': None, 'vid': None, 'tst': [], 'gam': None}
    ...
    for c in ["День рождения", "Свадьба", "Годовщина", "Рождение ребенка", "Свидание", "День влюбленных", "Новый год", "Девичник", "8 Марта", "Коллеге"]:
        k.insert(Btn(c, callback_data="cat_ok"))
    bot.edit_message_text("Выберите категорию:", uid, call.message.message_id, reply_markup=k)
elif data == "cat_ok":
    k = InlineKeyboardMarkup(row_width=2)
    for t in ["Ему", "Ей", "Семейная пара", "Группа мужчин", "Группа Женщин", "Женщины и Мужчины"]:
    ...
    bot.edit_message_text("Выбрано тостов: " + str(len(users[uid]['tst'])) + "/5", uid, call.message.message_id, reply_markup=call.message.reply_markup)
elif data == "tst_done":
