...
<html>
  ...
  <body style="margin: 0; overflow: hidden;">
    ...
    <!-- vr-mode-ui="enabled: false" убирает кнопку VR очков -->
    <a-scene embedded arjs="sourceType: webcam; debugUIEnabled: false;" vr-mode-ui="enabled: false">
      <a-assets>
        <!-- Убрали crossorigin, добавили preload -->
        <video id="ar-video" src="{{ video_url }}" playsinline webkit-playsinline loop preload="auto"></video>
      </a-assets>

      <!-- Видео привязано к камере и висит в пространстве перед пользователем -->
      <a-entity camera>
...
def home(subpath=""):
    # 1. Жестко перехватываем запросы к видеофайлам и отдаем именно файл!
    if subpath.startswith('static/'):
        filename = subpath.replace('static/', '', 1)
        return send_from_directory('static', filename)

    # 2. Обрабатываем ID коробки
    ...
    # 3. Если кода нет в базе — отправляем в бота привязывать контент
    if q not in db:
        return redirect("https://t.me/my_magic_ar_bot?start=" + str(q))
    
    # 4. Показываем AR сцену
    d = db[q]
    
    # Правильно склеиваем путь к видео. Если в базе '1', то будет '/static/video1.mp4'
    vid_id = str(d.get('vid', '1'))
    video_url = f"/static/video{vid_id}.mp4" 
