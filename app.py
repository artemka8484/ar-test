...
msg = "Настройка кода: " + qr_id + "\n\nВыберите язык:"
bot.send_message(uid, msg, reply_markup=k)
else:
...
bot.edit_message_text(text_menu, uid, call.message.message_id, reply_markup=get_hub_menu(uid))

elif data == "hub_reset":
qr_id = users[uid]['qr']
users[uid] = {'qr': qr_id, 'lang': None, 'vid': None, 'tst': [], 'gam': None}
k_lang = InlineKeyboardMarkup().add(Btn("🇷🇺 Русский", callback_data="lang_ru"))
bot.edit_message_text("Сброшено. Выберите язык:", uid, call.message.message_id, reply_markup=k_lang)

elif data == "hub_done":
qr_id = users[uid]['qr']
db[qr_id] = users[uid]
del users[uid]
fin_msg = "Готово! Всё привязано к коду " + qr_id
bot.edit_message_text(fin_msg, uid, call.message.message_id)

elif data in ["hub_vid", "hub_tst"]:
users[uid]['current_flow'] = "vid" if data == "hub_vid" else "tst"
k = InlineKeyboardMarkup(row_width=2)
cats = [
"День рождения", "Свадьба", "Годовщина", 
"Рождение ребенка", "Свидание", "День влюбленных",

Artem:
"Новый год", "Девичник", "8 Марта", "Коллеге"
]
for c in cats:
...
k = InlineKeyboardMarkup(row_width=2)
targs = [
"Ему", "Ей", "Семейная пара", 
"Группа мужчин", "Группа Женщин", "Женщины и Мужчины"
]
for t in targs:
...
bot.edit_message_text("Выберите до 5 тостов. Выбрано: 0/5", uid, call.message.message_id, reply_markup=k)
...
count_text = "Выбрано тостов: " + str(len(users[uid]['tst'])) + "/5"
bot.edit_message_text(count_text, uid, call.message.message_id, reply_markup=call.message.reply_markup)
